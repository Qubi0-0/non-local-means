...
# @staticmethod
# @jit(nopython=True) 
def non_local_means(input, neighbour_window_size, patch_window_size,h,layers):
                    ...
                    for patch_x in range(win_x,win_x + neighbour_window_size - patch_window_size):
                        for patch_y in range(win_y,win_y + neighbour_window_size - patch_window_size):
                            ...
                            if percent_completed % 5 == 0:
                                    print('Completed : ', percent_completed,'of layer nubmer: ',layer+1)
                    pix_val /= weight_sum
                    if layers == 3:
...
if __name__ == '__main__':
    ...
    if len(result.shape) > 2:
        layers = result.shape[2]
    else: 
        layers = 1
        result = np.expand_dims(result, axis=2)

    result_final = non_local_means(result,20,6,16,layers)
